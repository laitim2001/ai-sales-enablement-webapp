 ...
 * @module components/knowledge/enhanced-knowledge-search
 * @description
 * ================================================================AI銷售賦能平台 - 增強版知識庫搜索組件 (components/knowledge/enhanced-knowledge-search.tsx)================================================================【組件功能】Week 6 增強版搜索組件，集成高級語義分析、上下文感知和智能建議功能提供下一代搜索體驗，包括實時建議、結果增強、分析洞察和個性化推薦【主要職責】• 智能查詢建議 - 實時搜索建議和查詢補全• 語義搜索增強 - 深度語義理解和意圖識別• 上下文感知搜索 - 基於用戶上下文的個性化搜索• 結果增強顯示 - 智能摘要、關係和行動建議• 搜索分析洞察 - 實時搜索分析和用戶洞察• 多輪對話支援 - 連續搜索會話和指代消解• 即時反饋收集 - 用戶滿意度和搜索效果追蹤• 個性化體驗 - 基於用戶偏好的搜索優化【新增功能】• GPT-4語義理解 - 複雜查詢的深度分析• 實時搜索建議 - 智能查詢補全和相關建議• 結果集群分析 - 自動分組和主題識別• 搜索洞察報告 - 即時分析和改進建議• 上下文記憶 - 會話級別的搜索上下文保持• 多維度評分 - 質量、相關性、個性化綜合評分• 智能行動建議 - 基於結果的下一步建議• 實時性能監控 - 搜索性能和用戶體驗追蹤【相關檔案】• 語義處理: lib/search/semantic-query-processor.ts• 結果增強: lib/search/contextual-result-enhancer.ts• 搜索分析: lib/search/search-analytics.ts• 搜索建議: lib/search/search-suggestions.tsWeek 6 開發階段 - Task 6.4: 搜索介面優化和用戶體驗
 *
 * @created 2025-10-08
...
// 導入新的Week 6功能 - TEMPORARILY DISABLED due to server-side dependencies
// TODO: Move these features to API routes for proper server/client separation
...
export function EnhancedKnowledgeSearch() {
  ...
  const performEnhancedSearch = async () => {
    ...
    try {
      ...
      // 2. 執行基礎搜索 (Sprint 6 Week 11 Day 2: 添加資料夾過濾)
      const searchData = {
        query: search.query.trim(),
        type: search.searchType,
        category: search.category || undefined,
        tags: search.tags.length > 0 ? search.tags : undefined,
        folder_id: search.folderId !== null && search.folderId !== undefined ? search.folderId : undefined,  // 資料夾ID
        include_subfolders: search.includeSubfolders,  // 是否包含子資料夾
        limit: 20,
