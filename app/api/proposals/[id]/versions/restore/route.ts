...
export async function POST(
  ...
  try {
    ...
    if (createBackup) {
      try {
        backupVersion = await versionControl.createVersion(
          proposalId,
          userId,
    ...
    // 執行回滾
    try {
      const restoredProposal = await versionControl.revertToVersion(
        proposalId,
        versionId,
        userId
      );

      // TODO: 記錄回滾原因 - Proposal模型沒有metadata欄位
      // 可以考慮在ProposalVersion中添加回滾記錄
