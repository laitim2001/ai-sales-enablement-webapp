/**
 * @fileoverview AI銷售賦能平台知識庫主API路由 - 實現知識庫列表查詢和項目創建功能
 * @module app/api/knowledge-base/route
 *
 * ## 功能說明
 * 提供知識庫核心CRUD操作,包含分頁列表查詢、項目創建、智能重複檢測、
 * 自動向量化處理和動態標籤管理等完整功能。
 *
 * ## API規格
 * - **端點**: `GET /api/knowledge-base` - 獲取知識庫列表
 *   - 查詢參數: page, limit, category, status, search, tags, sort, order
 *   - 響應: { success, data: [...], pagination: {...} }
 * - **端點**: `POST /api/knowledge-base` - 創建知識庫項目
 *   - 請求: { title, content?, category?, source?, author?, language?, metadata?, tags? }
 *   - 響應: { success, data: {...}, message }
 * - **狀態碼**: 200/201 (成功) | 400 (驗證錯誤) | 401 (未認證) | 500 (伺服器錯誤)
 *
 * ## 主要職責
 * - 分頁查詢 - 支援多條件過濾、搜索和排序的知識庫列表
 * - 內容去重 - 使用SHA-256哈希檢測重複文檔
 * - 標籤管理 - 自動創建新標籤、更新使用次數統計
 * - 向量化處理 - 自動觸發文檔向量化任務
 * - 事務安全 - 使用數據庫事務確保數據一致性
 * - RBAC權限 - 整合角色權限控制
 *
