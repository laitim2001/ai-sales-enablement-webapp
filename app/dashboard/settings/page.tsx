/**
 * @fileoverview ================================================================AI銷售賦能平台 - 系統設定頁面 (app/dashboard/settings/page.tsx)================================================================【檔案功能】提供用戶帳戶管理和個人化設定的統一界面，涵蓋個人資料、通知偏好、安全設定和系統偏好，支持多分頁管理和實時設定同步，為用戶提供完整的設定管理體驗。【主要職責】• 個人資料管理 - 基本資訊、頭像上傳、部門設定• 通知偏好配置 - 郵件/推播/簡訊通知開關控制• 安全設定管理 - 密碼更新、登入記錄查看• 系統偏好配置 - 語言、時區、日期格式、主題設定• 設定同步保存 - 各分頁獨立保存和驗證【頁面結構】• 標題區域 - 設定頁面主標題和功能說明• 分頁管理 - Tabs組件管理四個主要設定分類• 個人資料分頁 - 頭像、基本資訊、部門設定• 通知設定分頁 - 多種通知方式的開關控制• 安全設定分頁 - 密碼修改和登入歷史• 系統設定分頁 - 語言、時區等環境配置【分頁結構】● 個人資料(profile) - firstName, lastName, email, phone, department, bio● 通知設定(notifications) - 郵件/推播/簡訊/任務/客戶/系統通知● 安全設定(security) - 密碼修改、登入記錄查看● 系統設定(system) - 語言、時區、日期格式、主題【用戶流程】1. 進入設定頁面，默認顯示個人資料分頁2. 修改基本資訊、上傳頭像、設定部門3. 切換到通知分頁，配置通知偏好4. 設定安全選項，修改密碼、查看登入記錄5. 設定系統偏好，選擇語言、時區、主題6. 各分頁獨立保存設定【狀態管理】• profile: 個人資料狀態物件• notifications: 通知偏好狀態物件• systemSettings: 系統設定狀態物件• isLoading: 保存操作載入狀態【相關檔案】• hooks/use-auth.tsx - 用戶認證和資料hook• components/ui/tabs.tsx - 分頁管理UI組件• components/ui/card.tsx - 卡片容器UI組件• components/ui/input.tsx - 表單輸入組件• components/ui/switch.tsx - 開關控制組件• components/ui/select.tsx - 下拉選擇組件【開發注意】• 資料驗證：所有表單輸入需加入前端驗證和後端驗證• 狀態管理：需考慮設定更新的樂觀更新和錯誤回滾• 安全性：密碼修改需加入雙重驗證和加密傳輸• 權限控制：不同用戶角色可編輯的設定範圍不同• 響應式設計：表單在小螢幕需保持可用性
