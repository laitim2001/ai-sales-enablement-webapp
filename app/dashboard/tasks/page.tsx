/**
 * @fileoverview ================================================================AI銷售賦能平台 - 任務管理頁面 (app/dashboard/tasks/page.tsx)================================================================【檔案功能】提供銷售團隊的任務管理和追蹤中心，支持任務創建、狀態更新、優先級管理和分類檢視，整合客戶資訊和績效統計，為用戶提供全面的工作流管理和協作平台。【主要職責】• 任務狀態管理 - 待處理/進行中/已完成三種狀態管理• 優先級系統 - 高/中/低優先級設定和視覺化顯示• 任務統計監控 - 各狀態任務數量統計和分析• 客戶關聯管理 - 任務與客戶的關聯展示和管理• 時間管理 - 截止日期設定和週期提醒• 標籤分類系統 - 任務標籤和分類管理【頁面結構】• 標題和操作區 - 頁面標題 + 篩選/新增按鈕• 統計卡片區 - 四個統計卡片显示任務数量和狀態• 分頁管理區 - Tabs組件管理不同狀態任務查看• 任務列表區 - 任務卡片列表和詳細信息展示【功能特色】• 狀態管理 - pending(待處理)/in_progress(進行中)/completed(已完成)• 優先級視覺化 - high(紅色)/medium(黃色)/low(綠色)色彩編碼• 動態統計 - 數量統計和百分比分析• 客戶關聯 - 任務與客戶資訊的關聯展示• 日期管理 - 截止日期追蹤和排序• 標籤系統 - 多標籤支持和分類管理【用戶流程】1. 查看任務統計卡片，了解整體任務狀況2. 選擇分頁查看特定狀態的任務3. 檢視任務詳情，包括描述、優先級、截止日期4. 使用篩選功能縮小查看範圍5. 點擊新增任務創建新的工作項目6. 使用更多操作菜單管理任務【數據結構】• Task界面 - id, title, description, priority, status, dueDate, assignee, customer?, tags[]• 優先級類型 - 'high' | 'medium' | 'low'• 狀態類型 - 'pending' | 'in_progress' | 'completed'【相關檔案】• components/ui/tabs.tsx - 分頁管理UI組件• components/ui/card.tsx - 任務卡片UI組件• components/ui/badge.tsx - 狀態和優先級標籤• components/ui/button.tsx - 操作按鈕組件【開發注意】• 性能優化：大量任務需考慮虛擬化和分頁載入• 實時更新：任務狀態變更需即時反映在UI上• 權限控制：不同用戶可編輯任務的範圍不同• 排序功能：任務列表支持多種排序方式• 通知提醒：截止日期接近時的自動提醒機制
