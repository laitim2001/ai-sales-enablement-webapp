...
describe('EncryptionService', () => {
  ...
  describe('加密資料格式和完整性', () => {
    ...
    it('篡改加密資料應該導致解密失敗', () => {
      ...
      }).toThrow(/integrity check failed|Decryption failed/i);
    });

    it('使用錯誤版本的加密資料應該拋出錯誤', () => {
      const plaintext = 'Test data';
      const encrypted = encryptionService.encrypt(plaintext);
  ...
  describe('批量欄位加密/解密', () => {
    ...
    it('批量加密應該跳過空值和非字符串欄位', () => {
      ...
      const encrypted = encryptionService.encryptFields(data, ['email', 'phone', 'age', 'active'] as any);
  ...
  describe('實際使用場景', () => {
    it('應該成功加密客戶敏感資料', () => {
      ...
      // 加密敏感欄位
      const encrypted = encryptionService.encryptFields(customer, ['email', 'phone', 'address']);
      ...
      // 解密驗證
      const decrypted = encryptionService.decryptFields(encrypted, ['email', 'phone', 'address']);
    ...
    it('應該成功處理提案內容加密', () => {
      ...
      // 加密機密內容
      const encrypted = encryptionService.encryptFields(proposal, ['content', 'customer_notes']);
      ...
      // 解密
      const decrypted = encryptionService.decryptFields(encrypted, ['content', 'customer_notes']);
